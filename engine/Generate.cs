    ...
    public static int Run(string[] args)
    {
        ...
        if (scan || modCat)
        {
            ...
            if (modCat)
            {
                ...
                if (bm) Console.WriteLine($"Base master {targetMod}: {ex.ByCategoryCount} NPCs covered by the prefix categories and {ex.UniqueCount} unique named characters (all_males path) are left to their dedicated paths.");
